...
async function askForQuantity(product) {
    const requestedQuantity = await number({
        message: `How many ${product.productName} would you like to order?`,
        validate: (value) => (value > 0 ? true : 'Please enter a positive number.')
    });
    if (requestedQuantity > product.quantity) {
        console.log(
            `Sorry, we only have ${product.quantity} ${product.productName}(s) in stock.`
        );
        return null
    } else {
        product.quantity -= requestedQuantity;
        console.log(`You have ordered ${requestedQuantity} ${product.productName}(s). Remaining quantity: ${product.quantity}.`);
        return true
    }
}
...
async function startShop() {
    ...
    await showProducts()
    const userInput = await askForInput()
    const validatedOrder = await validateInput(userInput)
    
    if (validatedOrder) {
        const product = products.find(p => p.productName === validatedOrder)
        await askForQuantity(product)
    }
}
